...
await window.set(window.ref(window.db, "orders"), orders);      
  console.log("✅ Data saved to Firebase");
    ...
    const users = [
        { username: "manager", password: "123456", warehouse: "P&C" },
        { username: "pharmaUser", password: "123456", warehouse: "Pharma" },
        { username: "retailUser", password: "123456", warehouse: "Retail" }
    ];
const warehouseSheets = [

    {
        name: "P&C",
        url: "https://docs.google.com/spreadsheets/d/e/2PACX-1vSAeWlFZdvqQqrWCq0uJKqxz6boomvVuNal1IYM1tOuoeraNE_ZW2BfYYKr3lKfmldOWOgWAXhz88Ke/pub?gid=228371932&single=true&output=csv"
    },

    {
        name: "Pharma",
        url: "https://docs.google.com/spreadsheets/d/e/2PACX-1vSAeWlFZdvqQqrWCq0uJKqxz6boomvVuNal1IYM1tOuoeraNE_ZW2BfYYKr3lKfmldOWOgWAXhz88Ke/pub?gid=875574333&single=true&output=csv"
    },

    {
        name: "Retail",
        url: "https://docs.google.com/spreadsheets/d/e/2PACX-1vSAeWlFZdvqQqrWCq0uJKqxz6boomvVuNal1IYM1tOuoeraNE_ZW2BfYYKr3lKfmldOWOgWAXhz88Ke/pub?gid=274206&single=true&output=csv"
    },

    {
        name: "Beesline",
        url: "https://docs.google.com/spreadsheets/d/e/2PACX-1vSAeWlFZdvqQqrWCq0uJKqxz6boomvVuNal1IYM1tOuoeraNE_ZW2BfYYKr3lKfmldOWOgWAXhz88Ke/pub?gid=1278749068&single=true&output=csv"
    },

    {
        name: "L'oreal lux",
        url: "https://docs.google.com/spreadsheets/d/e/2PACX-1vSAeWlFZdvqQqrWCq0uJKqxz6boomvVuNal1IYM1tOuoeraNE_ZW2BfYYKr3lKfmldOWOgWAXhz88Ke/pub?gid=884068351&single=true&output=csv"
    }
    ...
    const packingSheetURL ="https://docs.google.com/spreadsheets/d/e/2PACX-1vSAeWlFZdvqQqrWCq0uJKqxz6boomvVuNal1IYM1tOuoeraNE_ZW2BfYYKr3lKfmldOWOgWAXhz88Ke/pub?gid=457547473&single=true&output=csv";
async function loadAllWarehouseSheets() {
...
const responses = await Promise.allSettled(
    ...
    warehouseSheets.map(sheet =>
        fetch(sheet.url + "&t=" + Date.now(), {
            cache: "no-store"
        })
    )
...
for (const res of responses) {
    ...
    const h = parsed.data.shift();

    if (!headers.length)
        headers = h;

    mergedRows.push(...parsed.data);
}
// إذا لم ينجح تحميل أي Sheet لا تمسح البيانات الحالية


if (successCount === 0 || mergedRows.length === 0) {

    console.warn("⚠️ All warehouse sheets are empty. Keeping previous data.");
...
const ORDER_COL = headers.findIndex(h =>
    ...
    // ✅ 🔥 احفظ في Firebase
  if (canceledOrdersSet.size > 0) {
    await saveCanceledToFirebase([...canceledOrdersSet]);
}}
async function saveCanceledToFirebase(canceledArray) {
    const canceledRef = window.ref(window.db, "canceledOrders");

    const snapshot = await window.get(canceledRef);
    const existing = snapshot.val() || [];

    // دمج القديم + الجديد
    const merged = [...new Set([...existing, ...canceledArray])];

    await window.set(canceledRef, merged);

    console.log("✅ Canceled merged & saved");
}
function loadCanceledFromFirebase() {
    const canceledRef = window.ref(window.db, "canceledOrders");

    window.onValue(canceledRef, snapshot => {
        const data = snapshot.val();

        if (!data) return;

        canceledOrdersSet = new Set(data);

        console.log("✅ Canceled loaded from Firebase");
    });
}
    ...
    // DD/MM/YYYY HH:mm:ss
    const match2 = v.match(
        /^(\d{1,2})\/(\d{1,2})\/(\d{4})(?:\s+(\d{1,2}):(\d{2}):(\d{2}))?$/
    );

    if (match2) {
        const [, d, m, y, h = "00", min = "00", s = "00"] = match2;
...
async function loadPackingSheet() {

    try {

        const res = await fetch(
            packingSheetURL + "&t=" + Date.now(),
            {
                cache: "no-store"
            }
        );
        ...
        if (!rows.length) {
            console.warn("Packing sheet empty");
            return;
        }
        ...
        console.log("Packing Headers:", headers);


        // البحث عن Order Number
        const ORDER_COL = headers.findIndex(h =>
            h.includes("order")
        );


        // البحث عن وقت الـ Packing
        const PACK_TIME_COL = headers.findIndex(h =>
            h.includes("packing") ||
            h.includes("pack time") ||
            h.includes("packed time") ||
            h.includes("pack station")
        );
        ...
        if (ORDER_COL === -1) {
            ...
            console.warn(
                "Order column not found in Packing Sheet"
            );
        ...
        rows.forEach(r => {
            ...
            // إضافة الطلب إلى قائمة Packed
            newSet.add(orderNo);



            // استخراج وقت الـ Packing
            if (
                PACK_TIME_COL !== -1 &&
                r[PACK_TIME_COL]
            ) {
        ...
        // تحديث البيانات
        packingOrdersSet = newSet;

        packingTimeMap = newTimeMap;


        console.log(
            "✅ Packing Orders Loaded:",
            packingOrdersSet.size
        );

        console.log(
            "✅ Packing Times Loaded:",
            Object.keys(packingTimeMap).length
        );
    ...
    async function loadData({ force = false, silent = false } = {}) {
        ...
        try {
            ...
            await loadPackingSheet();// 🔥 هذا السطر كان ناقص 
const data = await loadAllWarehouseSheets();

if (!data) {
    console.warn("Keeping previous dashboard data.");
    return;
}
...
const headers = data.headers.map(h => h.toLowerCase().trim());
            ...
            const PACK_TIME = headers.findIndex(h =>
    h.includes("pack station")
);
            const map = {};
            rows.forEach(r => {
...
const isPacked =
    w.packed ||
    (packedRaw && packedValues.includes(packedRaw)) ||
    packingOrdersSet.has(orderNo);
...
if (isPacked) {
    ...
    map[orderNo].wh[w.base].packingTime =
        packingTimeMap[orderNo] || null;
...
saveOrdersToFirebase(newOrders);
            updateDashboard();
    ...
    orderNo,
    r[PACK_TIME],
    formatDateForInput(r[PACK_TIME])
);
    }
...
const todayDates = new Set(
    todayFiltered.map(o =>
        (getEffectiveDate(o) || "").split(" ")[0]
    )
);

const todayOrderNos = new Set(
    todayFiltered.map(o => o.orderNo)
);
...
backlogOrders = allOrders.filter(o => {
    ...
    const orderDate = dateToCheck.split(" ")[0];

    if (orderDate < ACCUMULATE_FROM) return false;

    // استبعاد أي طلب موجود في Today Orders
    if (todayOrderNos.has(o.orderNo)) return false;
...
`;
   ...
   backlogOrders.forEach(order => {
    const fullDate = getEffectiveDate(order);
...
`;
    ...
    let currentInterval = 300000; // يبدأ 30 ثانية  
    // ==============================  
    // 🔥 ENTERPRISE LIVE ENGINE  
    // ==============================  

    const FAST_INTERVAL = 600000;   // نشط  
    const SLOW_INTERVAL = 600000;  // خمول  
    ...
    function showNewOrderTab() {
        ...
        if (userWarehouse) {
            ...
            warehouseInput.readOnly = true; // يمنع التعديل
        }
        setTodayForNewOrder()
    }
...
const newOrder = {
    ...
    date: formattedDate,
    time: formattedTime,   // 🔥 إضافة الوقت
    createdAt: now.toISOString(), // وقت كامل دقيق للفرز
    warehouses: [{
        base: warehouse,
        packed: false,
...
.catch(err => {
    console.error("Sheet Error:", err);
        ...
        await saveOrdersToFirebase([...allOrders, newOrder]);
...
function clearNewOrderForm() {
    ...
    // LOAD SECURE ORDERS
    // =============================
    function loadSecureOrders() {

        const stored = localStorage.getItem("secureOrders");
        if (!stored) return;

        try {
            const orders = JSON.parse(simpleDecrypt(stored));
